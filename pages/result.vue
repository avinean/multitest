<template>
  <div class="flex-1 flex flex-col py-4 justify-center items-center min-h-screen">
    ...
    <div v-else-if="data.questions.length > 0" class="max-w-2xl w-full p-8 bg-white border border-gray-200 rounded-lg shadow-sm text-center">
      ...
      <div class="my-8">
        <div class="inline-flex items-center justify-center w-32 h-32 border-8 border-primary-500 rounded-full mb-4">
          ...
          <span class="text-xl">/ {{ data.questions.length }}</span>
        </div>
        <p class="text-2xl font-semibold">{{ Math.round((score / data.questions.length) * 100) }}%</p>
      </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto mt-8 p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
      ...
      <div class="space-y-8">
        <div v-for="(question, index) in data.questions" :key="index" class="border-b border-gray-200 pb-6 last:border-b-0">
...
const { data, pending: loading, error } = useDocument(
...
const score = computed(() => {
  let correct = 0
  data.value.questions.forEach((question, index) => {
